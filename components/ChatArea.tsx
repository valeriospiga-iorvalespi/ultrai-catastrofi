      ...
      <div style={{ padding: "14px 18px", borderBottom: "1px solid #e0e0e0",
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>
            {chunks.length} chunk dal normativo
          </div>
        </div>
...
export default function ChatArea({ productId, conversationId, onConversationUpdate }: ChatAreaProps) {
  ...
  // ✅ traccia l'ultimo conversationId caricato per evitare reload inutili
  ...
  // ✅ FIX: carica i messaggi quando cambia conversationId
  useEffect(() => {
  ...
  // ✅ FIX: salva i messaggi ogni volta che cambiano
  useEffect(() => {
    if (!conversationId) return;
    if (loadedConvId.current !== conversationId) return;
  ...
  const handleSend = useCallback(async () => {
    ...
    // Aggiorna il titolo della conversazione con il primo messaggio
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#fff" }}>
      ...
      <div style={{ borderTop: "1px solid #e0e0e0", padding: "12px 24px 14px", background: "#fff" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
          {[
            {
              label: "⬇️ Esporta chat",
              action: () => {
                const c = messages.map((m) => `[${m.role === "user" ? "Tu" : "UltrAI"} – ${m.timestamp.toLocaleTimeString("it-IT")}]\n${m.content}`).join("\n\n---\n\n");
                handleExport(c);
              },
            ...
            {
              label: "+ Nuova chat",
              action: () => { setMessages([]); setInputText(""); },
              disabled: false,
            },
          ].map((btn) => (
