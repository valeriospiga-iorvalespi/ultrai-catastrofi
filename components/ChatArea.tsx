// components/ChatArea.tsx — responsive
"use client";
...
interface ChatAreaProps {
  ...
  isMobile?: boolean; // ✅ NUOVO
}

function toStoredMessage(m: Message): StoredMessage {
  return { id: m.id, role: m.role, content: m.content, sourceIds: m.sourceIds, timestamp: m.timestamp.toISOString() };
}

function fromStoredMessage(m: StoredMessage): Message {
  return { id: m.id, role: m.role, content: m.content, sourceIds: m.sourceIds, timestamp: new Date(m.timestamp) };
}
...
function renderContent(text: string, onCitationClick: (id: string) => void): React.ReactNode {
  ...
  const flushList = () => {
    if (listItems.length > 0) {
      nodes.push(<ul key={`ul-${keyIdx++}`} style={{ margin: "6px 0 6px 18px", padding: 0 }}>{listItems}</ul>);
      listItems = [];
    }
  };
  lines.forEach((line, i) => {
    const k = `line-${i}`;
    if (/^##\s+/.test(line)) { flushList(); nodes.push(<div key={k} style={{ fontWeight: 700, fontSize: 14.5, color: "#003781", margin: "12px 0 4px" }}>{renderInline(line.replace(/^##\s+/, ""), onCitationClick, k)}</div>); return; }
    if (/^###\s+/.test(line)) { flushList(); nodes.push(<div key={k} style={{ fontWeight: 600, fontSize: 13.5, color: "#2c3e50", margin: "8px 0 2px" }}>{renderInline(line.replace(/^###\s+/, ""), onCitationClick, k)}</div>); return; }
    if (/^[-•]\s+/.test(line)) { listItems.push(<li key={k} style={{ marginBottom: 3, lineHeight: 1.55 }}>{renderInline(line.replace(/^[-•]\s+/, ""), onCitationClick, k)}</li>); return; }
    if (!line.trim()) { flushList(); nodes.push(<div key={k} style={{ height: 6 }} />); return; }
    flushList();
    nodes.push(<div key={k} style={{ lineHeight: 1.6, marginBottom: 2 }}>{renderInline(line, onCitationClick, k)}</div>);
  });
  flushList();
  return <>{nodes}</>;
}
    ...
    <div style={{
      ...
      <div style={{ padding: "14px 18px", borderBottom: "1px solid #e0e0e0",
        ...
        <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 20,
          cursor: "pointer", color: "#888", padding: "4px 8px", borderRadius: 6, lineHeight: 1 }}>×</button>
      </div>
...
function AssistantBubble({ message, onCopy, onExport, onShowSources, onCitationClick, isMobile }: {
  ...
  onShowSources: (ids: string[]) => void; onCitationClick: (chunkId: string) => void; isMobile?: boolean;
}) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{
        ...
        fontSize: 14, lineHeight: 1.6, color: "#2c3e50",
        ...
        <div>{renderContent(message.content, onCitationClick)}</div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, paddingLeft: 4, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1 }} />
        {[
          { label: copied ? "✓" : "📋", title: copied ? "Copiato" : "Copia", action: handleCopy, active: copied },
          { label: "👍", title: "Utile", action: () => setLiked(liked === "up" ? null : "up"), active: liked === "up" },
          { label: "👎", title: "Non utile", action: () => setLiked(liked === "down" ? null : "down"), active: liked === "down" },
        ].map((btn) => (
...
export default function ChatArea({ productId, conversationId, onConversationUpdate, isMobile = false }: ChatAreaProps) {
  ...
  useEffect(() => {
    ...
    if (currentConvId.current === conversationId) return;
    ...
    setTimeout(() => { isLoadingConv.current = false; }, 100);
  }, [conversationId]);

  useEffect(() => {
    if (!conversationId) return;
    if (isLoadingConv.current) return;
    if (currentConvId.current !== conversationId) return;
    saveMessages(conversationId, messages.map(toStoredMessage));
  ...
  const handleShowSources = useCallback(async (ids: string[]) => {
    ...
    try {
      ...
      if (res.ok) { const data = await res.json(); setSourcesPanel(data.chunks || []); }
      else { setSourcesPanel(ids.map((id) => ({ chunk_id: id, heading: id, text: "Testo non disponibile." }))); }
    } catch { setSourcesPanel(ids.map((id) => ({ chunk_id: id, heading: id, text: "Errore nel caricamento." }))); }
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ question: text, productId, history: messages.map((m) => ({ role: m.role, content: m.content })) }),
      });
      const data = await res.json();
      setMessages((prev) => [...prev, {
        id: (Date.now() + 1).toString(), role: "assistant",
        content: data.answer || "Mi dispiace, non ho trovato una risposta pertinente.",
        sourceIds: data.sources ?? [], timestamp: new Date(),
      }]);
    } catch {
      setMessages((prev) => [...prev, {
        id: (Date.now() + 1).toString(), role: "assistant",
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    // Su mobile Enter non invia (va a capo), su desktop invia
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#fff", minWidth: 0 }}>
      ...
      <div style={{
        ...
        padding: isMobile ? "10px 12px 12px" : "12px 24px 14px",
        background: "#fff",
      }}>
        <div style={{ border: "1px solid #c8d4e8", borderRadius: 10, overflow: "hidden" }}
          ...
          <textarea ref={textareaRef} value={inputText}
            ...
            style={{
              ...
              fontSize: isMobile ? 15 : 14, // 15px su mobile per evitare zoom iOS
              fontFamily: "inherit", color: "#2c3e50", lineHeight: 1.5,
              maxHeight: isMobile ? 120 : 160, background: "transparent",
            }} />
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "6px 10px 8px", gap: 6, alignItems: "center" }}>
            {!isMobile && (
            ...
            <button onClick={handleSend} disabled={!hasText || loading} title="Invia"
              style={{
                background: hasText && !loading ? "#003781" : "#c8d4e8", border: "none",
                borderRadius: 6,
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          ...
          <button onClick={() => { setMessages([]); setInputText(""); }}
            style={{ border: "1px solid #003781", borderRadius: 6, padding: "6px 12px",
